  ...
  if (playerHp <= monsterHp) {
      if (playerHp <= monsterMaxAtk) {
          if (playerDefense < 0.2) {
              // 플레이어 방어력이 낮으면 일반 공격
              return monster.actions[0].DoAction(monster, player); 
          } else {
              // 방어력이 높다면 몬스터는 회복을 시도
              return monster.actions[2].DoAction(monster, player); 
          }
      } else {
          if (monsterHp - playerHp >= monsterMaxAtk) {
              if (Math.random() < 0.8) {
                  // 체력 차이가 크면 연속 공격 시도
                  return monster.actions[1].DoAction(monster, player); 
              } else {
                  // 낮은 확률로 회복을 시도
                  return monster.actions[2].DoAction(monster, player); 
              }
...
const battle = async (stage, player, monster) => {
  ...
  let player_actions_text = player.actions.map(action => {
    const probabilityWithLuck = (100.0 - MyMath.Clamp( (action.probability - player.stats.luck),0.0,1.0) * 100.0).toFixed(2);
...
player_actions_text = TextTable.FormatText('action_info', {actions: player_actions_text});
  ...
  while(true) {
    ...
    if(command_result !== false){
      ...
      if(monster.stats.current_hp <= 0){//승리
        break;
      }
      const monster_result = monsterAction(player, monster);
      logs.push(... monster_result);
      monster_result.forEach( desc => {console.log(desc)});
    ...
    if (await Input.keyInYN(TextTable.FormatText('use_question'))) {
     ...
     let stat_delta = player.stats.atk_range.max_atk - max_atk;
      ...
      for(const [key, value] of Object.entries(changes)){
        TextTable.Output('buff', {stat_name: key, value: `${key}(${(value === '' ? '' : `+${value}`)})`});
        await Utils.Delay(500);
      }
    }
...
const  victoryScenario = async (player, stage) => {
  ...
  for(let i = 0; i < 4; i++){
    ...
    switch (i) {
      case 0:
        statIncrease = Math.round( player.stats.default_atk * stage *  MyMath.RandomRange(0.1, 0.2)); // 공격력 증가
        statName = 'MIN_ATK';
